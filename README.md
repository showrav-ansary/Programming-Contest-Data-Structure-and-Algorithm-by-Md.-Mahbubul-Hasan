...
You are requested to create an `issue` if you find any solution not well explained or wrong. You are expected to be language agnostic about the solutions though it will heavily be written in `C` or `C++` depending on when I wrote the solution and what was more convenient for me. You are requested to focus on explaining as much as possible.  

Please check [issues](https://github.com/showrav-ansary/Programming-Contest-Data-Structure-and-Algorithm-by-Md.-Mahbubul-Hasan/issues) to avoid duplication of working on the same problems
