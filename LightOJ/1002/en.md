...
* [WikiPedia](https://en.wikipedia.org/wiki/Graph_(abstract_data_type) "Graph (abstract data type) - WikiPedia")

* [WikiPedia](https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm "Dijkstra's algorithm - WikiPedia")

* [AbdulBari](https://www.youtube.com/watch?v=XB4MIexjvY0 "Abdul Bari's Explanation of Dijsktra - YouTube")


## Solution

At first we will simply create a _graph_ structure for the _Area Map_ in any preferred method (adj. matrix/ linked list). The problem statement has confirmed : (1) there shall be no negative cost for the roads and (2) the graph is a _bi-directional_/_undirected graph_. We can apply _Dijsktra's Algorihtm_ or its derivative or any similar. While the problem statement has stated the there are __multiple source nodes__ and actually the __destination node__ is the __home town__ but for ease of implementation we will consider the __home town__ as the __source node__ and the _other cities_ as the __destination nodes__, meaning it's not about finding the shortest path for _traversing all the cities in one go in one single optimal path_ rather it's about finding _multiple optimal path for each individual city by traversing multiple path_. Thus instead of being __greedy__, we go full __brute force__ not leaving any route for a single _city_ untried. More clearly, we won't be marking any _city_ for being traversed previously. We must implement in such way that we have to keep the _distance_ _array_/_list_ for keeping record of the _maximum road weight_ encountered while traversing through a specific route for a specific destination instead of _summing_ each _weight_ encountered and taking the _minimum sum_. Meaning, the `optimal path = path that has the minimum highest weighted node among all possible routing`. Look through the code for better understanding.

__Caution__ : Remember to use fast I/O for your preferred language as per the suggestion from the problem statement and find out what are the factors that disrupt them. 
...
int main()
{
    ...
    */
    ...
    for (int i = 1; i <= testCases; i++)
    {
        ...
        vector<int> areaMap[numberOfCities];      //The actual graph representing whether any road among them exist or not
        int distanceFromHomeTown[numberOfCities]; //The output array
        int cost[numberOfCities][numberOfCities]; //road costs

        memset(cost, 0, sizeof(cost)); //Setting the traversal status of all the city to `0` so it may hit false

        //Setting the initial max distance of other cities from the hometown to infinity/`Impossible`
        ...
        priority_queue<pair<int, int>> cityQueue; //making a queue to traverse through each of the city

        cityQueue.push(make_pair(0, homeTown)); //pushing the home town as our start point along with the cost
        ...
        while (!cityQueue.empty())
        {
            ...
            //taking the <cost, city> pair
            pair<int, int> startingCity = cityQueue.top();
            ...
            */
            ...
            for (int i = 0; i < areaMap[startingCity.second].size(); i++)
            {
                int currentCity = areaMap[startingCity.second][i];
                maxCostFound = max(distanceFromHomeTown[startingCity.second],
                                   cost[startingCity.second][currentCity]);
                if (distanceFromHomeTown[currentCity] > maxCostFound)
                {
                    distanceFromHomeTown[currentCity] = maxCostFound;
                    cityQueue.push(make_pair(-maxCostFound, currentCity)); //negative value used to make index smaller for priority
                }
            }
        }
